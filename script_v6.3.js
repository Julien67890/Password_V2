/* ═══════════════════════════════════════════════════════════════
   ...
   ✅ 219 mots simplifiés (faciles à taper et deviner)
   ✅ Tous les bugs corrigés
   ✅ Compatible Llama 3.2 (optionnel)
   ✅ PWA prête
   ✅ 100% local
   ═══════════════════════════════════════════════════════════════ */
...
function startNewManche() {
  // ✅ CORRECTIF: Réinitialiser mancheResults
  STATE.mancheResults = [];
  STATE.givenClues = [];
  console.log('🎮 Nouvelle manche démarrée');
...
function endManche() {
  ...
  // ✅ CORRECTIF: Ajouter les mots restants non traités
  const processedWords = STATE.mancheResults.map(r => r.word);
  STATE.mancheQueue.forEach(wordName => {
    if (!processedWords.includes(wordName)) {
      STATE.mancheResults.push({
        word: wordName,
        found: false,
        clues: 0
      });
      console.log('➕ Ajout mot non traité:', wordName);
    }
  });
